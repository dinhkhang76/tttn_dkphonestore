    ...
    public class XCart
    {
        ...
        public void DelCart(int? productid = null)
        {
            if (productid != null)
            {
                if (!System.Web.HttpContext.Current.Session["MyCart"].Equals(""))
                {
                    List<CartItem> listcart = (List<CartItem>)System.Web.HttpContext.Current.Session["MyCart"]; //Ép kiểu
                    int vt = 0;
                    foreach (var item in listcart)
                    {
        ...
        public List<CartItem> getCart()
        {
            ...
            return (List<CartItem>)System.Web.HttpContext.Current.Session["MyCart"]; //Ép kiểu
        }
    }
}